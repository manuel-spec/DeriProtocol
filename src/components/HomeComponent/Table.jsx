  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setFetchedData(Gainers.slice(0, 15));
    ...
    <div className="overflow-hidden max-h-440 rounded-lg shadow">
      <div className="flex flex-row justify-between items-center bg-[#0F1720] w-full text-sm mt-2">
        <button
          ...
          onClick={() => {
            fetchedData.sort((a, b) => b["24H"] - a["24H"]);
            console.log(1);
          }}
        >
          Top Gainers
        ...
        <button className="text-white p-2 mr-2">24H Turn Over</button>
      </div>
